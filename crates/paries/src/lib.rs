    ...
    fn draw(&mut self, id: OutputId, qhandle: &QueueHandle<Self>) {
        ...
        if output.render_surface.is_none() {
            ...
            let render_surface = if let Some(renderer) = &mut self.renderer {
                // SAFETY: The GPU surface is removed before its Wayland output surface is destroyed.
                unsafe { renderer.add_surface(&native, size) }
...
impl Dispatch<WlSurface, OutputId> for Wallpaper {
    fn event(_: &mut Self, _: &WlSurface, _: wl_surface::Event, _: &OutputId, _: &Connection, _: &QueueHandle<Self>) {}
}
